          ...
          <div className="register-form
            form--dark">
              ...
              heading--with-decoration">
                <h2 className="heading-title">Add Liquidity
                </h2> &nbsp; &nbsp; &nbsp;{" "}
                <span onClick={() => setOpenAllLiquidity
                (false)}>X</span>
              </header>
              ...
              label-floating is-empty">
                <select 
                  onChange={(e) => {
                    const selected = poolHistory?.atfind(
                    ...
                    <option>Select Item</option>
                    {
                      ...
                      (
                        <option key={index + 1} value={index + 1}>
                          {pool.token_A.name} & {pool.token_B.symbol}
                          Fee: {""}{" "}
                          {pool.fee}
                        </option>
                      ))
              ...
              <Input placeholder={"liquidity amount"} handleClick={(e) => 
              setLiqudityAmount(e.target.value)}
              />
              <Input placeholder={"approve amount"} handleClick={(e) => 
              setApproveAmount(e.target.value)}
              />
              <button onClick={() => CREATE_LIQUIDITY(
                selectedPool, liqudityAmount, approveAmount
              )}
